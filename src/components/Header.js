    ...
    <header className="sticky top-0 z-50 bg-white/80 backdrop-blur">
      <div className="container-pad flex items-center justify-between py-3">
        ...
        <nav className="hidden md:flex items-center gap-8 text-sm">
          {nav.map(i => <Link key={i.href} href={`${i.href}?lang=${lang}`} className="hover:text-fondeka-green">{i.label}</Link>)}
          ...
          <Link href={`#download?lang=${lang}`} className="btn btn-primary">{t('cta.download')}</Link>
        </nav>
      ...
      {open && (
        <div className="md:hidden border-t bg-white">
          <div className="container-pad py-3 flex flex-col gap-3">
            {nav.map(i => <Link key={i.href} href={`${i.href}?lang=${lang}`} className="py-2">{i.label}</Link>)}
